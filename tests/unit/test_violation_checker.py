...
def make_person(x1=0, y1=0, x2=100, y2=200) -> Detection:
    return make_detection("Person", x1=x1, y1=y1, x2=x2, y2=y2)
...
def test_violation_when_no_person_but_confident_no_x(checker):
...
def test_no_violation_for_box_not_on_a_person(checker):
    ...
    with patch("time.time", return_value=1000.0):
        checker._get_state(1).get(None, "NO-Hardhat").last_safe_time = 990.0
